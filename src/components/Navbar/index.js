            ...
            <div className={`px-3 h-14 md:h-16 items-center   ${scroll < 50 ? 'bg-transparent' : "bg-light  dark:bg-dark "} ${Style.navFont}  w-full flex  `}>
                ...
                <div id="center" className="hidden  lg:flex font-semibold text-xs    justify-center">
                    {navs.map((ele, index) => {
                        ...
                        return <Link key={index} href={ele.href} ><a key={index} className={`   ${index ? "border-black dark:border-white border-l-1 border-opacity-50" : ""} px-5   hover:opacity-50  cursor-pointer uppercase  `}> <span>{ele.name}</span>
                        </a></Link>
                    })}
                </div>
